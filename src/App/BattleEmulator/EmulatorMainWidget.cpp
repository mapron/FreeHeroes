    ...
    , m_adventureKingdom(std::make_unique<AdventureKingdom>())
{
    ...
    m_ui->comboBoxPositionsPreset->addItems({ tr("Standard"),
...
int EmulatorMainWidget::execBattle(bool isReplay, bool isQuick)
{
    ...
    if (!isReplay) {
        ...
        if (result == QDialog::Accepted) {
            ...
            resultInfo.resultDescription = resultInfo.goodResult ? tr("Glorious victory!<br>For the valor in the battle, %1 recieves %2 experience").arg(resultInfo.sides[0].name).arg(defenderHPloss) : tr("Your forces have suffered a crushing defeat, and %1 now leaves you").arg(resultInfo.sides[0].name);
