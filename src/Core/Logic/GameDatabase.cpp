    ...
    : m_impl(std::make_unique<Impl>())
{
    ...
    for (auto* spell : m_impl->m_spells.m_unsorted) {
        ...
        if (spell->type == LibrarySpell::Type::Offensive || spell->type == LibrarySpell::Type::Summon)
            spell->tags.push_back(LibrarySpell::Tag::OffensiveSummon);
    ...
    for (auto* unit : m_impl->m_units.m_unsorted) {
        ...
        if (0) {
            auto name = unit->presentationParams.name.ts.contains("ru_RU") ? unit->presentationParams.name.ts.at("ru_RU") : unit->untranslatedName;
            Logger(Logger::Warning) << name << "\t" << unit->guardMult100 << "%\t" << (10000 / unit->guardMult100);
        }
    }
    for (auto* unit : m_impl->m_units.m_unsorted) {
    ...
    for (auto* artifact : m_impl->m_artifacts.m_unsorted) {
        ...
        if (artifact->statBonus.nonEmptyAmount())
    ...
    auto         makePlanarMask = [](const std::vector<uint8_t>& data, bool invert) -> PlanarMask {
        PlanarMask res;
    ...
    auto makeCombinedMask = [](const PlanarMask& blockMask, const PlanarMask& visitMask) -> CombinedMask {
        CombinedMask res;
    ...
    for (auto* obj : m_impl->m_objectDefs.m_unsorted) {
        ...
        obj->combinedMask   = makeCombinedMask(obj->blockMapPlanar, obj->visitMapPlanar);
