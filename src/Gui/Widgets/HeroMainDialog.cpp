    ...
    , m_ui(std::make_unique<Ui::HeroMainDialog>())
{
    ...
    for (auto * pb : {m_ui->pushButtonClose,
         ...
         m_ui->pushButtonBag, m_ui->pushButtonListInfo}) {
...
void HeroMainDialog::updateHeroAppearence()
{
    ...
    m_ui->labelExperienceValue->setProperty("popupDescr", expInfo.descr
                                           .arg(m_hero->getSource()->level)
                                           .arg(FormatUtils::formatLargeInt(m_hero->getSource()->experience))
                                           .arg(FormatUtils::formatLargeInt(m_hero->getSource()->estimated.experienceNextLevel))
                                           .arg(FormatUtils::formatLargeInt(remainExp)));
    ...
    m_ui->labelManaValue->setProperty("popupDescr", manaInfo.descr
                                       .arg(m_hero->getSource()->mana).arg(m_hero->getSource()->estimated.maxMana));
