...
struct GUIWIDGETS_EXPORT SporadicHandle {
    ...
    SporadicHandle(const SporadicConfig& cfg)
        : cfg(cfg)
    {
        reset();
    }
...
public:
    ...
    struct AnimGroupSettings {
        ...
        AnimGroupSettings(int group, int durationMs)
            : group(group)
            , durationMs(durationMs)
        {}

        AnimGroupSettings& setReverse(bool val = true)
        {
            reverse = val;
            return *this;
        }
        AnimGroupSettings& setLoopOver(bool val = true)
        {
            loopOver = val;
            return *this;
        }
        AnimGroupSettings& setStartFrame(int frame)
        {
            startFrame = frame;
            return *this;
        }
        AnimGroupSettings& setRepeatFrame(int onFrame, int count)
        {
            repeatOnFrame      = onFrame;
            repeatOnFrameCount = count;
            return *this;
        }
...
public:
    ...
    void setDrawOriginH(DrawOriginH drawOrigin)
    {
        m_drawOriginH = drawOrigin;
        update();
    }
    void setDrawOriginV(DrawOriginV drawOrigin)
    {
        m_drawOriginV = drawOrigin;
        update();
    }
...
class GUIWIDGETS_EXPORT SpriteItemObj : public QObject
    , public SpriteItem {
    Q_OBJECT
