...
template<> bool qMapLessThanKey<FreeHeroes::Gui::IGraphicsLibrary::PixmapKey>(const FreeHeroes::Gui::IGraphicsLibrary::PixmapKey &key1, const FreeHeroes::Gui::IGraphicsLibrary::PixmapKey &key2)
{
    return std::tie(key1.resourceName, key1.group, key1.frame) < std::tie(key2.resourceName, key2.group, key2.frame);
}

template<> bool qMapLessThanKey<FreeHeroes::Gui::IGraphicsLibrary::PixmapKeyList>(const FreeHeroes::Gui::IGraphicsLibrary::PixmapKeyList &key1, const FreeHeroes::Gui::IGraphicsLibrary::PixmapKeyList &key2)
{
    const bool isRangeLess = std::lexicographical_compare(key1.cbegin(), key1.cend(),
                                                          key2.cbegin(), key2.cend(),
                                                          [](const auto & l , const auto & r){ return qMapLessThanKey(l, r);});
    return isRangeLess;
}
...
public:
    ...
    AsyncSprite(GraphicsLibrary::Impl * impl, const std::string & resourceName)
        : m_impl(impl), m_resourceName(resourceName) {}
...
public:
    ...
    AsyncPixmap(GraphicsLibrary::Impl * impl, const PixmapKey & resourceCode)
        : m_impl(impl), m_resourceCode(resourceCode) {}
...
public:
    ...
    AsyncIcon(GraphicsLibrary::Impl * impl, const PixmapKeyList & resourceCodes)
        : m_impl(impl), m_resourceCodes(resourceCodes) {}
...
struct GraphicsLibrary::Impl
{
    ...
    Impl(Core::IResourceLibrary & resourceLibrary) : m_resourceLibrary(resourceLibrary) {
    }
};
...
bool GraphicsLibrary::AsyncSprite::exists() const { return m_impl->isSpriteResourceExists(m_resourceName);}

bool GraphicsLibrary::AsyncSprite::isLoaded() const { return m_impl->m_spriteCache.contains(m_resourceName);}

bool GraphicsLibrary::AsyncSprite::preload() const { return m_impl->getSyncObjectAnimation(m_resourceName) != nullptr;}

SpritePtr GraphicsLibrary::AsyncSprite::get() const { return m_impl->getSyncObjectAnimation(m_resourceName);}

bool GraphicsLibrary::AsyncPixmap::exists() const { return m_impl->isSpriteResourceExists(m_resourceCode.resourceName);}

bool GraphicsLibrary::AsyncPixmap::isLoaded() const { return m_impl->m_pixmapCache.contains(m_resourceCode);}

bool GraphicsLibrary::AsyncPixmap::preload() const { return !m_impl->getSyncPixmapByKey(m_resourceCode).isNull();}

QPixmap GraphicsLibrary::AsyncPixmap::get() const { return m_impl->getSyncPixmapByKey(m_resourceCode);}

bool GraphicsLibrary::AsyncMovie::exists() const { return m_impl->isVideoResourceExists(m_resourceName);}

bool GraphicsLibrary::AsyncMovie::isLoaded() const { return true;}

bool GraphicsLibrary::AsyncMovie::preload() const { return true;}

QMovie* GraphicsLibrary::AsyncMovie::create(QObject* parent) const { return m_impl->getSyncVideo(m_resourceName, parent);}
...
bool GraphicsLibrary::AsyncIcon::isLoaded() const { return m_impl->m_iconCache.contains(m_resourceCodes);}

bool GraphicsLibrary::AsyncIcon::preload() const { return !m_impl->getSyncIcon(m_resourceCodes).isNull();}

QIcon GraphicsLibrary::AsyncIcon::get() const { return m_impl->getSyncIcon(m_resourceCodes);}
